...
export function LiveQueue({ albums, onAction, maxPhotosPerAlbum = 5 }: LiveQueueProps) {
          ...
          filteredAlbums.map(album => (
            ...
            >
              <div className="p-4 flex items-center gap-4">
                {/* Status Bar */}
                <div className={`w-1 h-10 rounded-full ${album.status === 'completed' ? 'bg-emerald-500' :
                    ...
                    <div className="flex items-center gap-2 w-full max-w-[120px]">
                      <div className="h-1.5 flex-1 bg-zinc-800 rounded-full overflow-hidden">
                        <div
                          ...
                          style={{ width: `${Math.min(((album.photo_count || 0) / maxPhotosPerAlbum) * 100, 100)}%` }}
                        />
                      </div>
                      <span className="text-xs font-mono text-white">{album.photo_count || 0}/{maxPhotosPerAlbum}</span>
                    </div>
                  </div>

                  {/* Payment Status - Check both status and payment_status */}
                  <div className="hidden md:block">
                    <div className={`text-xs font-medium inline-flex items-center gap-1.5 px-2 py-1 rounded-md ${(album.payment_status === 'paid' || album.status === 'paid') ? 'bg-emerald-500/10 text-emerald-400' : 'bg-zinc-800 text-zinc-400'
                      }`}>
