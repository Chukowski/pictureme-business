...
export const Component = React.forwardRef<HTMLButtonElement, AnimatedButtonProps>(({ className, children, speed = "3s", ...props }, ref) => {
    return (
        ...
        >
            ...
            />
            <span className="inline-flex h-full w-full cursor-pointer items-center justify-center rounded-full bg-card text-sm font-medium text-white backdrop-blur-3xl hover:bg-card/90 transition-colors">
                {children}
            </span>
        </button>
    );
