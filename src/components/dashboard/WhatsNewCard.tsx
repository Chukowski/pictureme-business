    ...
    return (
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
            <Card className="bg-[#080808] border-white/5 rounded-2xl md:rounded-3xl overflow-hidden group shadow-2xl shadow-black/50">
                <CardHeader className="pb-3 pt-4 px-4 md:px-6 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
                    <div className="flex items-center gap-2 w-full md:w-auto">
                        <div className="w-8 h-8 md:w-7 md:h-7 rounded-full bg-[#D1F349]/10 flex items-center justify-center shrink-0">
                            <Zap className="w-4 h-4 md:w-3.5 md:h-3.5 text-[#D1F349] fill-[#D1F349]" />
                        </div>
                        <CardTitle className="text-base md:text-xl font-black tracking-tight text-white uppercase leading-none">What's New</CardTitle>
                    </div>

                    <TabsList className="bg-white/5 border border-white/5 h-8 p-0.5 w-full max-w-sm md:w-fit">
                        <TabsTrigger value="features" className="flex-1 md:flex-none text-[8px] md:text-[9px] font-bold uppercase tracking-wider h-7 px-3 md:px-5 data-[state=active]:bg-[#D1F349] data-[state=active]:text-black">Features</TabsTrigger>
                        <TabsTrigger value="templates" className="flex-1 md:flex-none text-[8px] md:text-[9px] font-bold uppercase tracking-wider h-7 px-3 md:px-5 data-[state=active]:bg-[#D1F349] data-[state=active]:text-black">Tips</TabsTrigger>
                        <TabsTrigger value="other" className="flex-1 md:flex-none text-[8px] md:text-[9px] font-bold uppercase tracking-wider h-7 px-3 md:px-5 data-[state=active]:bg-[#D1F349] data-[state=active]:text-black">Alerts</TabsTrigger>
                    </TabsList>
                </CardHeader>

                <CardContent className="px-4 md:px-6 pb-5 md:pb-6 pt-1">
...
function AnnouncementItem({ announcement, onCTA, onImageClick }: {
    ...
    return (
        <div
            className={`relative p-4 md:p-5 rounded-2xl border border-white/5 hover:border-white/10 transition-all duration-300 group/item overflow-hidden ${hasImage ? 'cursor-pointer' : 'bg-white/[0.02] hover:bg-white/[0.04]'
                }`}
            ...
            <div className="relative z-10 flex items-start justify-between gap-4">
                <div className="space-y-2 w-full">
                    <div className="flex flex-wrap items-center gap-2">
                        <h4 className={`text-[13px] md:text-base font-black uppercase tracking-tight transition-colors ${hasImage
                    ...
                    <p className={`text-[12px] md:text-xs leading-relaxed max-w-full line-clamp-2 ${hasImage ? 'text-zinc-200' : 'text-zinc-400'
                        }`}>
